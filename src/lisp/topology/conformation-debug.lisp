...
(defun fill-internals-from-fragments-debug (conf fragments &optional index)
  ...
  (let ((monomer-to-conformation-selector (make-hash-table))
    ...
    (loop with atagg = (ataggregate conf)
          ...
          for frags = (gethash monomer-context (monomer-context-to-context-rotamers fragments))
          for monomer-position = (gethash monomer (monomer-positions conf))
          for monomer-index = (residue-index monomer-position)
          for atres = (elt (atresidues atmol) monomer-index)
          for context-rotamers = (let ((fp (gethash monomer-context (monomer-context-to-context-rotamers fragments))))
                                (unless fp
                                  (error "Cannot find context ~a in conformations" monomer-context))
                                fp)
          for conformations = (fragments context-rotamers)
                 ...
                 (if (null previous-monomer)
                     ...
                     (let* ((previous-conformation (elt (consistent-conformations previous-conformation-selector)
                       ...
                       (loop for conf in (all-conformations conformation-selector)
                             ...
                             for dihedral0 = (/ (topology:dihedral-rad internal0) 0.0174533)
                             for dihedral1 = (/ (topology:dihedral-rad internal1) 0.0174533)
                             for dihedral2 = (/ (topology:dihedral-rad internal2) 0.0174533)
  ...
  (loop with atagg = (ataggregate conf)
        ...
        for context-rotamers = (let ((fp (gethash monomer-context (monomer-context-to-context-rotamers fragments))))
                              (unless fp
                                (error "Cannot find context ~a in conformations" monomer-context))
                              fp)
        for _fp = (format t "context-rotamers = ~a~%" context-rotamers)
        for rand-limit = (length (fragments context-rotamers))
        for rand-index = (if index index (random rand-limit))
        for fragment-internals = (let ((fragments (fragments context-rotamers)))
                                   (unless fragments
                                     (error "fragments is NIL for context ~a" monomer-context))
                                   (elt (fragments context-rotamers) rand-index))
