...
(defun fill-internals-from-fragments (conf fragments &optional (index 0))
  ...
  (loop with atagg = (ataggregate conf)
        ...
        for fragment-pool = (gethash monomer-context (monomer-context-to-fragment-pool fragments))
...
(defun search-conformations (oligomer-space fragment-pool-map monomer-names sdf-filename &optional (number-struct 50) (number-conf 100))
  (with-open-file (fout sdf-filename :direction :output)
    (loop for struct-count below number-struct
          ...
          do (topology::fill-internals-from-fragments conf fragment-pool-map 0)
             (loop for count below number-conf
                   do (topology::fill-internals-from-fragments-for-monomers-named conf fragment-pool-map monomer-names)
