...
(defun add-axes (component axesp)
  (let* ((amidx 0.0)
         ...
         (axes-cyl (cylinder-buffer (list (list aminx amidy amidz)
                                    ...
                                    (list (list amaxx amidy amidz)
                                          (list amidx amaxy amidz)
                                          (list amidx amidy amaxz))
                                    :color '((.8 0 0) (0 .8 0) (0 0 .8))))
         (axes-sph (sphere-buffer (list (list aminx amidy amidz)
                                        (list amidx aminy amidz)
                                        (list amidx amidy aminz)
                                        (list amaxx amidy amidz)
                                        (list amidx amaxy amidz)
                                        (list amidx amidy amaxz))
                                    :color '((.8 0 0) (0 .8 0) (0 0 .8) (.8 0 0) (0 .8 0) (0 0 .8))))
         (axes-text (text-buffer (list (list (+ amaxx +default-radius+) amidy amidz)
                                       (list amidx (+ amaxy +default-radius+) amidz)
                                       (list amidx amidy (+ amaxz +default-radius+)))
                                 '("x" "y" "z")
                                 :color '((.8 0 0) (0 .8 0) (0 0 .8)))))
    (setf (ngl:representations component)
          (append (ngl:representations component)
