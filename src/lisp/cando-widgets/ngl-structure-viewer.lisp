...
(defun on-picked (instance picked)
  ...
  (when (and (equal (gethash "signal" picked) "click")
             (equal (gethash "type" picked) "bond"))
    (let* ((bond (gethash "bond" picked))
           (name (gethash "component" picked))
           ...
           (atom1 (aref atom-vec (gethash "atom_index1" bond)))
           (atom2 (aref atom-vec (gethash "atom_index2" bond)))
