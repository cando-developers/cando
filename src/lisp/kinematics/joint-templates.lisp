...
(defun maybe-make-in-plug (in-plug-factory complex-plugs)
  ...
  (if (in-plug in-plug-factory)
      ...
      (let ((reference-atom (in-reference-atom in-plug-factory))
            (maybe-complex-plug (complex-plug-or-nil (in-name in-plug-factory) complex-plugs)))
        (if maybe-complex-plug
            (let* ((compiled-smarts (compiled-smarts maybe-complex-plug))
...
(defun maybe-make-out-plug (out-plug-factory complex-plugs)
  ...
  (if (out-plug out-plug-factory)
      ...
      (let ((reference-atom (out-reference-atom out-plug-factory))
            (maybe-complex-plug (complex-plug-or-nil (out-name out-plug-factory) complex-plugs)))
        (if maybe-complex-plug
            (let* ((compiled-smarts (compiled-smarts maybe-complex-plug))
                   (matches (chem:matches compiled-smarts reference-atom)))
              (unless matches
...
Return a list of prepare-topology objects - one for each residue that we need to extract."
  (let* ((chemdraw-object (chemdraw design))
    ...
    (loop for prepare-topology in extracted-prepare-topologys
          ...
          for plugs = (plugs prepare-topology)
          for constitution = (constitution prepare-topology)
          for joint-template = (build-internal-coordinate-joint-template name residue plugs constitution)
          do (setf (joint-template prepare-topology) joint-template))
...
(defun get-all-stub-pivot-atoms-as-set (plugs out-plug-bond0-atoms-set )
  (declare (ignore out-plug-bond0-atoms-set))
...
(defun one-atom-as-string (me constitution-atoms)
  ;; joint-template-atom-name is no longer available
  #+(or)(let ((target (make-string-output-stream))
        (result (make-string-output-stream)))
    (format target "~a" (joint-template-atom-name me constitution-atoms))
    (format target "/~a" (class-name (class-of me)))
    (format result "~30a: " (get-output-stream-string target))
    (loop for child in (children me)
          do (let ((child-name (joint-template-atom-name child constitution-atoms)))
               (format result "~a/~a" child-name (joint-template-comment child))))
...
(defmethod write-into-joint-tree ((joint-template complex-bonded-joint-template) parent-joint atresidue atmolecule-index atresidue-index)
  (let* ((constitution-atoms-index (constitution-atoms-index joint-template))
         (atom-name (atom-name joint-template))
         (atomid (list atmolecule-index atresidue-index constitution-atoms-index))
         (joint (make-complex-bonded-joint atomid atom-name))
...
(defun build-internal-coordinate-joint-template-tree-spanning-tree (name residue plugs constitution root-atom all-spanning-atoms back-spanning-map)
;;;chemdraw-fragment constitution-atoms plugs constitution-name topology-name)
  (let* ((in-plug (let ((in-plug (find-in-plug plugs)))
    ...
    (dolist (atom all-spanning-atoms)
      (let ((parent-atom (gethash atom back-spanning-map)))
        (when parent-atom
...
(defun build-internal-coordinate-joint-template (name residue plugs constitution)
  (let* ((in-plug (let ((in-plug (find-in-plug plugs)))
                    (if in-plug
                        in-plug
                        (error "There has to be an in-plug in topology ~a" name))))
    ...
    (loop for atm in all-spanning-atoms
          for name = (chem:get-name atm)
          for back-span-atom = (chem:get-back-span spanning-loop atm)
          for back-span-name = (chem:get-name back-span-atom)
          do (setf (gethash atm back-spanning-map) back-span-atom))
    (build-internal-coordinate-joint-template-tree-spanning-tree name residue plugs constitution root-atom all-spanning-atoms back-spanning-map)))

(defparameter *max-index* 0)
#+(or)
(defun walk-joint-templates (node)
  (setf *max-index* (max *max-index* (id node)))
  (loop for child in (children node)
        do (walk-joint-templates child)))

#+(or)
(defun save-joint-templates (node vec)
  (format t "Setting ~a -> ~a~%" (id node) (name node))
  (setf (aref vec (id node)) (name node))
  (loop for child in (children node)
