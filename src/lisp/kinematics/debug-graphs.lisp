  ...
  (let ((nodes nil)
    ...
    (let ((edges nil))
      (chem:map-bonds 'nil
                      (lambda (atm1 atm2 bond-order)
                        (let* ((node1 (gethash atm1 atom-to-node-id))
                               (bond-order-char (case bond-order
                                                  (:single-bond #\s)
                                                  (:double-bond #\d)
                                                  (:triple-bond #\t)
                                                  (otherwise #\?)))
                                       ...
                                       (if node2-child-of-node1
                                           (let ((child-index (position atm2 children1)))
                                             (cytoscape:make-element
                                              ...
                                              :data (j:make-object "source" (string node2)
                                                                   "target" (string node1)
                                                                   "label" (format nil " ~a" bond-order-char))))
                                           (let ((child-index (position atm1 children2)))
                                             (cytoscape:make-element
                                              :group "edges"
                                              :data (j:make-object "source" (string node1)
...
(defmethod label (joint-template node-id)
  (format nil "~a~%~a" (class-name (class-of joint-template)) (atom-name joint-template)))

(defmethod label ((joint-template bonded-joint-template) node-id)
  (format nil "BONDJT~%~a" (atom-name joint-template)))

(defmethod label ((joint-template jump-joint-template) node-id)
  (format nil "JUMP~%~a" (atom-name joint-template)))

(defmethod label ((joint-template in-plug-bonded-joint-template) node-id)
  (format nil "IN~%~a~%~a" (atom-name joint-template) (in-plug joint-template)))

(defmethod label ((joint-template complex-bonded-joint-template) node-id)
  (let* ((ajoint (parent joint-template))
         (ajoint-name (when ajoint (string (atom-name ajoint))))
         (input-stub-joints (input-stub-joints joint-template))
         (bjoint (aref input-stub-joints 0))
         (bjoint-name (when bjoint (string (atom-name bjoint))))
         (cjoint (aref input-stub-joints 1))
         (cjoint-name (when cjoint (string (atom-name cjoint)))))
  (format nil "CBJT~%~a~%~a" (atom-name joint-template)
...
(defun order-name (bond)
  (etypecase bond
    (kin.fragment:single-bond "s")
    (kin.fragment:double-bond "d")
    (kin.fragment:ring-single-bond "rs")
    (kin.fragment:ring-double-bond "rd")
    ))
    
(defun do-render-fragment (fragment)
  (let ((atom-nodes (make-hash-table))
        nodes edges)
    (loop 
      for atom in (kin.fragment:atoms fragment)
      for atom-name = (kin.fragment:name atom)
      for id = (gensym)
      for node = (cytoscape:make-element :group "nodes"
                                         :data (j:make-object "id" (string id)
                                                              "label" (string atom-name)))
      do (setf (gethash atom-name atom-nodes) id)
      do (push node nodes)
      do (loop for bond in (kin.fragment:bonds atom)
               for target = (kin.fragment:target bond)
               for target-id = (gethash target atom-nodes)
               for edge = (cytoscape:make-element :group "edges"
                                                  :data (j:make-object "source" (string id)
                                                                       "target" (string target-id)
                                                                       "label" (order-name bond)))
               do (push edge edges)
               ))
    (values (gethash (kin.fragment:name (first (kin.fragment:atoms fragment))) atom-nodes)
            nodes
            edges)))

(defun render-fragment (fragment)
  (multiple-value-bind (root-id nodes edges)
      (do-render-fragment fragment)
    (make-instance 'cytoscape:cytoscape-widget
                   :graph-layouts (list (make-instance 'cytoscape:cose-layout
                                                       :roots (list (string root-id))))
                   :graph-style "* { label: data(label); font-size: 10;text-wrap: wrap; } edge { curve-style: straight; target-arrow-shape: triangle; }"
