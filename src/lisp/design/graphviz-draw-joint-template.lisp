...
(defmethod label (joint-template)
  (let ((name (string (class-name (class-of joint-template)))))
    (format nil "~a/~a[~a]" name (design.joint-tree:name joint-template) (design.joint-tree:id joint-template))))

(defmethod stream-draw-joint-template ((joint-template design.joint-tree:joint) stream)
