...
(defun recursively-write-into-atresidue (joint-template
                                         ...
                                         atmolecule-index
                                         atresidue-index)
  (let ((joint (design.joint-tree::write-into-joint-tree joint-template
                                                              parent-joint
                                                              atresidue
                                                              atmolecule-index
                                                              atresidue-index)))
    (loop for child-joint-template in (design.joint-tree::children joint-template)
  ...
  (let* ((topology (topology atresidue))
    ...
    (let* ((joint-template (chem:topology/get-property topology :joint-template))
           ...
           (root-joint (recursively-write-into-atresidue joint-template
                                                         ...
                                                         atmolecule-index
                                                         atresidue-index)))
      (unless (slot-boundp joint-tree 'kin:root)
        (setf (kin:root joint-tree) root-joint))
