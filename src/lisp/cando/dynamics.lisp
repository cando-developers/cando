...
(defclass simulation ()
  ((scoring-function :initarg :scoring-function :accessor scoring-function)
   ...
   (delta-t :initarg :delta-t :accessor delta-t :type (geom:vecreal-type))
...
(defun make-atomic-simulation (scoring-function &key (delta-t 1.0) accumulate-coordinates)
  (let* ((vector-size (chem:get-nvector-size scoring-function))
         (coordinates (make-array vector-size :element-type (geom:vecreal-type)))
         (velocity (make-array vector-size :initial-element 0.0 :element-type (geom:vecreal-type)))
         (forces (make-array vector-size :element-type (geom:vecreal-type)))
         (temp-forces (make-array vector-size :element-type (geom:vecreal-type)))
         (delta-t-over-mass (make-array (/ vector-size 3) :element-type (geom:vecreal-type)))
...
(defun velocity-verlet-step (dynamics &key (velocity-verlet-function #'chem:velocity-verlet-step) unfrozen)
