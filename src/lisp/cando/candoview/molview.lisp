...
(defun molview (&optional (mol ethanol))
  (setq curr-mol mol)
  (let ((window (make-instance 'mol-window)))
    (glut:display-window window)))


(defun view (aggregate &optional (style :line))
  (setf view-style style)
  (current-aggregate-set aggregate)
  (glut:post-redisplay))
...
(defun element-material (element)
  (case element
    ((:H) #(0.8 0.8 0.8 1))
    ((:O) #(0.8 0.1 0.0 1))
    ((:C) #(0.2 0.2 0.2 1))
    (otherwise #(0.5 0.5 0.0))))
...
(defun current-aggregate-set (aggregate )
    ...
    (chem:map-bonds
     ...
     (lambda (x y o b)
       ...
       (let ((x-element (chem:get-element x))
	     ...
	     (ypos (chem:get-position y)))
	 (let ((half-pos (geom:times-scalar (geom:v+ xpos ypos) 0.5)))
	   (unless (gethash x-element lines)
	     (push (cons xpos half-pos) (gethash x-element lines))
	     (push (cons ypos half-pos) (gethash y-element lines))))))
     aggregate)
    (setf *current-lines* lines)))

(defun draw-lines ()
  (maphash (lambda (atom lines)
             (gl:material :front :ambient-and-diffuse
                          (element-material element))
             (dolist (line lines)
               (let ((a (car line))
...
(defmethod glut:display ((window mol-window))
  ...
  (gl:with-pushed-matrix
      (gl:translate 0 0 view-dist)
    (if show-light-source (draw-light-source))
    (if (eq view-style :line)
	(draw-lines)
        (draw-atoms))
    (glut:swap-buffers)))
...
(defmethod glut:mouse ((window mol-window) button state x y)
  #+(or)(format t "glut:mouse button[~s] state[~s] @ ~a, ~a~%" button state x y)
  (cond
...
(defmethod glut:motion ((window mol-window) x y)
  (case motion
    (:rotate
     ...
     (glut:post-redisplay))
    (:scale
     (setf view-dist (+ origdist (* (- y (second origclick)) 0.5)))
     (lim-between view-dist -200 20)
     (format t "view-dist = ~s~%" view-dist))))
