  ...
  (let ((correction-map (make-hash-table))
    ...
    (multiple-value-bind (atom-number-vec element-vec charge-vec)
      ...
      (chem:map-atoms
       nil
       (lambda (a)
         (if (string= (chem:get-element a) (aref element-vec count))
             (setf (gethash a correction-map) (aref charge-vec count))
           #+(or)   (format t "atom name ~a element ~a element ~a~%" (chem:get-name a) (chem:get-element a) (aref atom-number-vec count)))
         (incf count))
       mol)
      correction-map)))
