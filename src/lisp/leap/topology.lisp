...
||#
    (multiple-value-bind (j-vec kbj-vec r0j-vec)
        (collapse-stretch-parameters (chem:atom-types energy-function) kb-vector r0-vector atom1-vector atom2-vector)
      (Let ((with-h 0)
            (without-h 0))
...
(defun prepare-amber-energy-angle (energy-function)
  (let* ((energy-angle (chem:get-angle-component energy-function))
    ...
    (multiple-value-bind (j-vec ktj-vec t0j-vec)
        (collapse-angle-parameters (chem:atom-types energy-function) kt-vector t0-vector atom1-vector atom2-vector atom3-vector)
    (Let ((with-h 0)
          (without-h 0))
...
(defun collapse-dihedral-parameters (atom-types v-vec in-vec phase-vec proper-vec atom1-vec atom2-vec atom3-vec atom4-vec)
  (let ((j-vec (make-array (length v-vec) :fill-pointer 0))
        (jnext 0)
        jtemp
        (uniques (make-hash-table :test #'equal))
        (vj-vec (make-array 256 :element-type (geom:vecreal-type) :fill-pointer 0 :adjustable t))
        (inj-vec (make-array 256 :element-type '(integer 1 6) :fill-pointer 0 :adjustable t))
        (phasej-vec (make-array 256 :element-type (geom:vecreal-type) :fill-pointer 0 :adjustable t))
    ...
    (loop for i from 0 below (length v-vec)
          ...
          for type1 = (chem:get-type atom1 atom-types)
          for type2 = (chem:get-type atom2 atom-types)
          for type3 = (chem:get-type atom3 atom-types)
          for type4 = (chem:get-type atom4 atom-types)
...
;          (ffnonbond1 (chem:get-ffnonbond-using-type-index ffnonbond-db type-index1))
          (cn1-vec (make-array (/ (* ntypes (+ ntypes 1)) 2) :element-type (geom:vecreal-type)))
          (cn2-vec (make-array (/ (* ntypes (+ ntypes 1)) 2) :element-type (geom:vecreal-type)))
          (type1 0)
          (type2 0)
          (rstar 0)
          (epsilonij 0))
...
;;;                    (format *debug-io* "In generate-nonbond-parameters i->~a j->~a type1->~a type2->~a~%" i j type1 type2)
                    (let ((ffnonbond1 (chem:get-ffnonbond-using-type-index ffnonbond-db type1))
                      ...
                      (setf rstar (+ (chem:ffnonbond/get-radius-angstroms ffnonbond1) (chem:ffnonbond/get-radius-angstroms ffnonbond2))
                            ...
                            (aref cn1-vec index) (geom:vecreal (* epsilonij (expt rstar 12.0)))
                            (aref cn2-vec index) (geom:vecreal (* 2.0 epsilonij (expt rstar 6.0)))
...
(defun chem:prepare-amber-energy-nonbond (energy-function ffnonbond-db)
  (let* ((atom-table (chem:atom-table energy-function))
         ...
         (charge-vector (make-array natom :element-type (geom:vecreal-type)))
         (mass-vector (make-array natom :element-type (geom:vecreal-type)))
         ...
         (atom-radius-vector (make-array natom :element-type (geom:vecreal-type)))
...
;;;    (format t "ffnonbond-db -> ~a~%" ffnonbond-db)
    (loop for i from 0 below natom
       ...
       do (setf (aref atom-name-vector i) atom-name
                ...
                (aref charge-vector i) (geom:vecreal charge)
                (aref mass-vector i) (geom:vecreal mass)
                (aref type-index-vector i) type-index
                (aref atomic-number-vector i) atomic-number
                (aref atom-radius-vector i) (geom:vecreal atom-radius)))
  ...
  (fortran:with-fortran-input-file (fif topology-pathname :direction :input)
    (let (natom ntypes nbonh mbona ntheth mtheta nphih mphia nhparm nparm
      ...
      (let ((energy-stretch (core:make-cxx-object 'chem:energy-stretch))
            ...
            ;; ... more of these
            (kbs-vec (make-array (+ nbonh mbona) :element-type (geom:vecreal-type)))
            (r0s-vec (make-array (+ nbonh mbona) :element-type (geom:vecreal-type)))
            ...
            (kta-vec (make-array (+ ntheth mtheta) :element-type (geom:vecreal-type)))
            (t0a-vec (make-array (+ ntheth mtheta) :element-type (geom:vecreal-type)))
            ...
            (vd-vec (make-array (+ nphih mphia) :element-type (geom:vecreal-type)))
            (ind-vec (make-array (+ nphih mphia) :element-type '(signed-byte 32)))
            (dnd-vec (make-array (+ nphih mphia) :element-type (geom:vecreal-type)))
            (phased-vec (make-array (+ nphih mphia):element-type (geom:vecreal-type)))
  ...
  (fortran:with-fortran-input-file (fif coordinate-filename :direction :input)
    ...
    (let* ((line (fortran:fread-line fif))
           ...
           (time (if (>= (length line) 15)
                     (fortran::parse-vecreal (subseq line 5 nil) :start 0 :end 15)
                     nil))
           (temp (if (>= (length line) 35)
                     (fortran::parse-vecreal (subseq line (+ 5 15) nil) :start 0 :end 15)
                     nil)))
      (fortran:fread-line fif)
      (let* ((coordinates (copy-seq (fortran:fread-vecreal-vector fif 6 12 (* natoms 3))))
             (velocities (when read-velocities
                           (copy-seq (fortran:fread-vecreal-vector fif 6 12 (* natoms 3)))))
             (bounding-box (when read-bounding-box
                             (copy-seq (fortran:fread-vecreal-vector fif 6 12 6)))))
...
(defun cell-lengths (netcdf)
  (let ((sv (static-vectors:make-static-vector 3 :element-type (geom:vecreal-type))))
...
(defun load-amber-topology-restart-pair (&key topology-filename coordinate-filename)
  (multiple-value-bind (energy-function aggregate)
    ...
    (multiple-value-bind (number-of-atoms coordinates bounding-box)
      ...
      (when bounding-box
        (setf (chem:bounding-box aggregate) bounding-box))
