...
(defun parse-ptor-line (line)
  (flet ((not-white-space-p (c) (char>= c #\!))
    ...
    (let* ((atom1 (intern (string-trim " " (subseq line 0 2)) :keyword))
           ...
           (barrier (fortran::read-double-float-from-string (subseq line barrier-start barrier-end)))
           ...
           (phase-deg (fortran::read-double-float-from-string (subseq line phase-start phase-end)))
           ...
           (periodicity (fortran::read-double-float-from-string (subseq line periodicity-start periodicity-end)))
