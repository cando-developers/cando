...
(defun tirun-calculation-from-ligands (ligands)
  (let* ((residue-name-to-pdb-alist nil)
         ...
         (calc (make-instance 'tirun:tirun-calculation
                              :ligands tirun-structures
                              :mask-method :closest
...
(defun bonded-atoms (atm diffs)
  (let (heavy-match heavy-diff hydrogens)
    (loop for bond in (chem:bonds-as-list atm)
          ...
          do (if (= (chem:atomic-number-for-element (chem:get-element other-atm)) 1)
                 (push atm hydrogens)
                 (if (member atm diffs)
                     (push atm heavy-diff)
                     (push atm heavy-match))))
...
(defun find-closest-hydrogen (atm hydrogens)
  (let* ((closest-hydrogen (first hydrogens))
    ...
    (loop for hyd in (second hydrogens)
                 ...
                 (multiple-value-bind (heavy2-match heavy2-diff hydrogen2)
                   ...
                   (if (/= (length hydrogen1) (length hydrogen2))
                       ...
                       (cond
                         ...
                         (t
                          (setf diff1 (push-all hydrogens1 diff1)
                                diff2 (push-all hydrogens2 diff2))))))))
