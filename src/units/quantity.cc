...
//
	...
	if ( obj->isAssignableTo<addon::OVector3_O>() ) return;
	...
	if ( obj->isAssignableTo<core::DoubleFloat_O>() )
	{
	    ...
	    return rval;
	} else if ( obj->isAssignableTo<addon::OVector3_O>() )
	{
	    addon::OVector3_sp oval = obj.as<addon::OVector3_O>();
	    addon::OVector3_sp nval = lisp->create<addon::OVector3_O>(oval->get().multiplyByScalar(conversion));
	    return nval;
    ...
    {_OF();
	addon::OVector3_sp element = this->getElement_in_unit(index,other).as<addon::OVector3_O>();
	return element->get();
    }
	...
	} else if ( other->isAssignableTo<addon::OVector3_O>() )
	{
	    if ( this->_Value->isAssignableTo<core::Number_O>() )
	    {
		addon::OVector3_sp vother = other.as<addon::OVector3_O>();
