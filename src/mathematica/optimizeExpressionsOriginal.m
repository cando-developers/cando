...
globalLastAbsoluteTime = AbsoluteTime[];
...
collectTermsForOneLevel[fAndTerms_] :=
        Module[{newExpression,lev,ts,nt,expr, nf,i},
	       nf = fAndTerms[[1]];
	       nt = fAndTerms[[2]];
	       (*nf = nf/.nt;*)
	       lev = Depth[nf];
		   ...
		   If[!((Head[expr]=== Integer) ||
			...
			(Head[expr] === Rational)),
		      ...
		      nt = Append[nt,newExpression];
                      (*Print["Head newExpression = ", Head[newExpression[[1]]] ];*)
                      If[(Head[newExpression[[1]]] === List ||
                          Head[newExpression[[1]]] === Greater),
                         Print["!!!!!! Bad newExpression ", CForm[newExpression], " ts = ", ts ];
                         Print["!!!!!! Bad newExpression  lev = ", lev, " nf = ", nf];
                      ];
		      PrintTemporary["Collected subexpression: ", CForm[newExpression]]; (*Comment out*)
		      nf = nf/.(expr -> map[expr]);
		   ];
	       ];
	       Return[{nf,nt}];
...
eliminateTrivialRules[packet_,outputs_] :=
        Module[{rules,trivRules,nonTrivRules,leftRules},
	       rules = Rules/.packet;
		  ...
		  Print["Trivial rules are being removed!"];
...
identifyRules[packet_] :=
        Module[{changes,pack,equations,newnt,result, nt,i,numberOfNewTerms},
               equations = Rules/.packet;
               For[i=1,i<=Length[equations],i++,
                   If[ Head[equations[[i]]]===Rule, (*If this is a rule then print the rule*)
                       Print["Would collect terms for rule: ",i, " ", equations[[i]][[2]] ];];
                   ];
        ];
...
collectTerms[packet_,outputs_] :=
        Module[{changes,pack,equations,newnt,result, nt,i,numberOfNewTerms,startTime},
               ...
               Print["collectTerms elapsed = ", AbsoluteTime[]-startTime, " seconds"];
               If[changes,Print["CHANGES MADE, RERUN"]];
               result
        ];
...
associativePrintComplexTerms[terms_,type_] :=
        Module[{allVars,allTerms,i,j,ot,allNt,nt},
...
associativeTermContains[assoc_,vars_] :=
        Module[{tl,i,res},
...
associativeSubstitute[rule_,vars_,newVar_,type_] :=
        Module[{tnew,newTimes,leftSide,rightSide,tl,i,newTerm},
...
associativeFirstUse[rules_,vars_,type_] :=
        Module[{i,use},
...
associativesSimplify[packet_,type_,outputs_] :=
        Module[{terms,newTerms,done,common,firstAbsoluteTime,oneAbsoluteTime},
	       ...
	       Print["associativeSimplify>>Came out of eliminateTrivialRules"];
...
cFormWriteToFiles[rules_,pack_] :=
        Block[{fileName,st,declares,code},
...
testCaseBlock[interaction_,pack_] :=
        Module[{results,args,rules,body,vars,v,str,termRules,i,result},
               rules = Rules/.pack;
...
matchExpEval[exp_,eval_] :=
        Module[{i,expNorm,evalNorm,dev},
...
locateCreateBranches[term_] :=
        Module[{i,mx,loc,branch,branches},
...
createRoot[rule_] :=
        Module[{lhs,rhs,tl,list,syms,branches,mx,branchNames,i,col,head},
	        lhs = rule[[1]];
	        rhs = rule[[2]];
...
buildTreeGraphics[asx_,asy_] :=
        Module[{ha,subBranches,tv,tvv,br,i,j,lines,graphics,color,line,txt},
	       graphics = {};
	       lines={};
	       ha=0.1;
	       For[i=1,i<=Length[treeValues],i++,
...
simplifyRules = {
        ArcTan[x_,y_] -> myatan2[y,x], (* C++ has reverse argument order *)
        Mod[x_,y_] -> mymod[x,y], (* otherwise CForm will convert this to % *)
...
packOptimize[pack_] :=
        Module[{np,outs,rules,code,result,mathExp,fileNamePrefix, fileName,cycles,startTime,skipTimesOptimize,skipPlusOptimize},
	       ...
	       np = pack;
               startTime = AbsoluteTime[];
	       Print[Style["Set SkipTimesOptimize and SkipPlusOptimize to turn these simplifications off and on",Blue,Italic,24]];
               shortCycles = (ShortOptimizeCycles/.pack)===True;
               If[shortCycles,
	          Print["Collecting terms short cycle 1/1"];
	          np = collectTerms[np,outs]/.simplifyRules;
	          Print["Collecting terms done short cycle 1/1"];
                ,
	          Print["Collecting terms cycle 1/4"];
	          np = collectTerms[np,outs]/.simplifyRules;
	          Print["Collecting terms cycle 2/4"];
	          np = collectTerms[np,outs]/.simplifyRules;
	          Print["Collecting terms cycle 3/4"];
	          np = collectTerms[np,outs]/.simplifyRules;
	          Print["Collecting terms cycle 4/4"];
	          np = collectTerms[np,outs]/.simplifyRules;
	          Print["Collecting terms done last cycle 4/4"];
               ];
               Print["Checking SkipTimesOptimize = ", SkipTimesOptimize/.pack ];
               skipTimesOptimize = (SkipTimesOptimize/.pack)===True;
	       If[!skipTimesOptimize,
                  Print[Style["Running timesSimplify", Blue, Italic, 20]];
		  np = timesSimplify[np,outs]/.simplifyRules;
		,
		  Print[Style["Skipping timesSimplify",Blue,Italic,20]];
	       ];
               If[shortCycles,
                  Print["Skipping terms cycle after TimesSimplify"];
                  ,
	          np = collectTerms[np,outs]/.simplifyRules;
               ];
               skipPlusOptimize = (SkipTimesOptimize/.pack)===True;
	       If[!skipPlusOptimize,
                  Print[Style["Running plusSimplify", Blue, Italic, 20]];
		  np = plusSimplify[np,outs]/.simplifyRules;
		,
		  Print[Style["Skipping plusSimplify",Blue,Italic,20]];
	       ];
	       rules = Rules/.np;
	       ...
	       Print["Writing mathematica expression to file:"];
	       Print["File: "<>fileNamePrefix];
...
writeFiniteDifferenceForce[sout_,pack_] :=
        Block[{inputs,derivs,i,vars1,vars2,svars1,svars2,d,str},
...
assembleFiniteDifferenceDiagonalHessian[pack_,d_] :=
        Block[{res,inputs,derivs,vars1,vars2,vars3,svars1,svars2,svars3},
	      inputs = Prepend[Input/.pack,ENERGY_FUNCTION];
	      derivs = DerivativeVariables/.pack;
	      vars1 = inputs/.{d->d-delta2};
	      vars2 = inputs;
...
assembleFiniteDifferenceOffDiagonalHessian[pack_,d1_,d2_] :=
        Block[{res,inputs,derivs,vmm,vmp,vpm,vpp,smm,smp,spm,spp},
	        inputs = Prepend[Input/.pack,ENERGY_FUNCTION];
	        derivs = DerivativeVariables/.pack;
...
writeFiniteDifferenceHessian[sout_,pack_] :=
        Block[{inputs,derivs,i1,i2,d1,d2,tst},
	      inputs = Input/.pack;
	      derivs = DerivativeVariables/.pack;
...
writeSerializeCode[pack_, vars_] :=
        Module[{fileName,st,var,i},
...
writeOutputVariablesForDebugging[pack_] :=
        Block[{var,vars,inps,st,i,outs,fileName},
	      inps = Input/.pack;
	      outs = Output/.pack;
...
ForceAccumulate[macroPrefix_,nameRules_,forceName_] :=
        CCode[macroPrefix<>"_FORCE_ACCUMULATE("<>ToString[Index/.nameRules]
<>", "<>ToString[Offset/.nameRules]<>", "<>ToString[forceName]<>" );"];


AppendChainGradientAndForce[macroPrefix_,be_,outputs_,rawEnergyFn_,chainVar_,chainFn_,rawNames_] :=
        Module[{i,gradName,zforceName,energyFn,nameInfo,varName,names,deriv},
	        Print["About to append Gradient and Force terms"];
		    ...
		    deriv = D[energyFn,chainVar]*D[chainFn,varName];
...
SetAttributes[AppendChainGradientAndForce,HoldAll];

AppendGradientAndForce[macroPrefix_,be_,outputs_,rawEnergyFn_,rawNames_] :=
        Module[{i,gradName,zforceName,energyFn,nameInfo,varName,names,deriv},
...
HessianAccumulate[macroPrefix_,name1Info_,name2Info_,hessianName_]:=
        CCode[macroPrefix<>"_HESSIAN_ACCUMULATE("
...
AppendChainOneHessianElement[macroPrefix_,be_,outputs_,hessianStructure_,energyFn_,chainVar_,chainFn_,names_,i_,j_] :=
        Module[{name1Info,name2Info,var1Name,var2Name,hessianName,hess,prefix,zfirstDeriv},
	       name1Info = ExtractNames[names[[i]]];
	       name2Info = ExtractNames[names[[j]]];
	       var1Name = Name/.name1Info;
	       var2Name = Name/.name2Info;
	       If[i==j,prefix="d"(*Diagonal*),prefix="o"];
	       hessianName = Symbol[prefix<>"h"<>ToString[var1Name]<>ToString[var2Name]];
               hess = D[D[energyFn,chainVar],chainVar]*D[chainFn,var1Name]*D[chainFn,var2Name]+D[energyFn,chainVar]*D[D[chainFn,var1Name],var2Name];
	       AppendTo[be,Assign[hessianName,hess]];
	       AppendTo[outputs,hessianName];
	       AppendTo[be,HessianAccumulate[macroPrefix,name1Info,name2Info,hessianName]];
	       Print["Appended hessian element ("<>ToString[j] <>","<>ToString[i]<>") for variable: "<>ToString[hessianName]];
	       hessianStructure[[i,j]]=Length[outputs];
	       If[i!=j,hessianStructure[[j,i]]=Length[outputs]];
        ];
SetAttributes[AppendChainOneHessianElement,HoldAll];


AppendChainHessianDiagonalElements[macroPrefix_,be_,outputs_,hessianStructure_,rawEnergyFn_,chainVar_,chainFn_,rawNames_] :=
        Module[{names,energyFn,i},
	        energyFn = Evaluate[rawEnergyFn];
	        names = Evaluate[rawNames];
	        For[i=1,i<=Length[names],i++,
		    AppendChainOneHessianElement[macroPrefix<>"_DIAGONAL",be,outputs,hessianStructure,energyFn,chainVar,chainFn,names,i,i];
	        ];
        ];
SetAttributes[AppendChainHessianDiagonalElements,HoldAll];
...
AppendHessianOffDiagonalElements[macroPrefix_,be_,outputs_,hessianStructure_,rawEnergyFn_,rawNames_] := Module[
	...
	For[col=1,col<Length[names],col++,
		For[row=col+1,row<=Length[names],row++,
			AppendOneHessianElement[macroPrefix<>"_OFF_DIAGONAL",
                                                be,outputs,hessianStructure,energyFn,names,col,row];
		];
	];
];
...
AppendChainHessianOffDiagonalElements[macroPrefix_,be_,outputs_,hessianStructure_,rawEnergyFn_,chainVar_,chainFn_,rawNames_] :=
        Module[{energyFn,name1Info,name2Info,var1Name,var2Name,hessianName,col,row,names},
	       names = Evaluate[rawNames];
	       energyFn = Evaluate[rawEnergyFn];
	       For[col=1,col<Length[names],col++,
	           For[row=col+1,row<=Length[names],row++,
		       AppendChainOneHessianElement[macroPrefix<>"_OFF_DIAGONAL",be,outputs,hessianStructure,energyFn,chainVar,chainFn,names,col,row];
		   ];
	       ];
        ];
SetAttributes[AppendChainHessianOffDiagonalElements,HoldAll];


(* Now do everything *)
AppendChainGradientForceAndHessian[macroPrefix_,be_,outputs_,hessianStructure_,rawEnergyFn_,chainVar_,chainFn_,rawVarNames_]:=
        Module[{energyFn,varNames},
               Print["Entered AppendChainGradientForceAndHessian"];
	       energyFn = Evaluate[rawEnergyFn];
	       varNames = Evaluate[rawVarNames];
	       AppendTo[be,CCode["#ifdef "<>macroPrefix<>"_CALC_FORCE //["]];
	       AppendTo[be,CCode["if ( calcForce ) {"] ];
	       AppendChainGradientAndForce[macroPrefix,be,outputs,energyFn,chainVar,chainFn,varNames];
...
SetAttributes[AppendChainGradientForceAndHessian,HoldAll];


(* Now do everything *)
AppendGradientForceAndHessian[macroPrefix_,be_,outputs_,hessianStructure_,rawEnergyFn_,rawVarNames_]:=
        Module[{energyFn,varNames},
	        energyFn = Evaluate[rawEnergyFn];
	        varNames = Evaluate[rawVarNames];
	        AppendTo[be,CCode["#ifdef "<>macroPrefix<>"_CALC_FORCE //["]];
...
(* For rigid body calculations *)
AppendGradientForceIfCalcForceExtraRule[macroPrefix_,be_,outputs_,rawEnergyFn_,rawVarNames_,extraRule_]:=
        Module[{energyFn,varNames},
	       energyFn = Evaluate[rawEnergyFn];
	       varNames = Evaluate[rawVarNames];
	       AppendTo[be,CCode["#ifdef "<>macroPrefix<>"_CALC_FORCE //["]];
