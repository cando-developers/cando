...
#include "addon/renderController.h"
...
{_OF();
    ...
    {
	addon::Color_sp color = atom->getPropertyOrDefault(kw::_sym_color,_Nil<addon::Color_O>()).as<addon::Color_O>();
	if ( color.nilp() )
	{
	    this->_Color = addon::Color_O::systemColor(kw::_sym_magenta);
	} else
	{
	    this->_Color = color;
	}
...
addon::Color_sp GrPickableMatter_O::elementToColor(Element element, core::Lisp_sp lisp)
{_G();
    core::Symbol_sp sym = _sym__PLUS_elementColors_PLUS_;
    addon::IntegerKeyObjectDictionary_sp elementColorMap = sym->symbolValue().as<addon::IntegerKeyObjectDictionary_O>();
    if ( elementColorMap->contains(element) )
    {
	return elementColorMap->get(element).as<addon::Color_O>();
    }
    return elementColorMap->get(element_Undefined).as<addon::Color_O>();
}
...
{_G();
    ...
    {
	if ( (*it)->isAssignableTo<addon::Render_O>() )
	{
	    ...
	    addon::Render_sp render = (*it).as<addon::Render_O>();
	    LOG(BF( "    (*it)->getParent().get() = %p")% (*it).as<addon::Render_O>()->getParent().get() );
...
addon::Render_sp GrPickableMatter_O::findChild(core::Cons_sp symbolPath, addon::RenderController_sp controller)
{_OF();
    if ( !this->isOn(controller) ) return _Nil<addon::Render_O>();
    if ( this->_Name.nilp() )
    {
	IMPLEMENT_ME();
    }
    ...
    {
	if ( symbolPath->car<core::Symbol_O>() == this->_Name ) return this->sharedThis<addon::Render_O>();
    }
...
void	GrPickableMatter_O::_walkAndDefineSliderRanges(addon::RenderController_sp controller)
{_G();
}

void	GrPickableMatter_O::_walkAndDefineController(addon::RenderController_sp controller,bool suppressSwitch)
{_G();
...
#if 0
addon::Render_O::iterator GrPickableMatter_O::beginRender(addon::RenderController_sp controller)
{
...
addon::Render_O::iterator GrPickableMatter_O::endRender(addon::RenderController_sp controller)
{
    return this->_PartsDisplayList.end();
}
#endif
...
addon::OVector3_sp GrPickableMatter_O::centerOfRenderedGeometry()
{_G();
...
{_G();
    ...
    this->_BondLines = addon::GrPickableLines_O::create();
	...
	addon::Color_sp c1 = ra1->getColor();
	addon::Color_sp c2 = ra2->getColor();
    ...
    {
	this->_Labels = addon::GrLabels_O::create();
	for ( ai=this->_Atoms.begin(); ai!=this->_Atoms.end(); ai++ )
	{
	    if ( (*ai)->getShowLabel() )
	    {
...
{_G();
    ...
    void GrPickableMatter_O::accumulateCenterOfGeometry(addon::CenterOfGeometryEnum cogType, addon::RenderController_sp rc, Vector3& sumPos, uint& sumPoints )
{
    ...
    void GrPickableMatter_O::accumulateBoundingBox(const Matrix& transform, addon::CenterOfGeometryEnum cogType, addon::RenderController_sp rc, addon::BoundingBox_sp bbox)
{
    if ( !this->isOn(rc) ) return;
    this->_AtomPoints->accumulateBoundingBox(transform,cogType,rc,bbox);
}

    void GrPickableMatter_O::accumulateRenderInformation(stringstream& ss, addon::RenderController_sp cc)
    {
	// nothing
    }
