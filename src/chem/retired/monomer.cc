...
//
	...
	for ( it=this->_Couplings.begin(); it!=this->_Couplings.end(); it++ ) {
		...
		coup = it->second.lock().as<DirectionalCoupling_O>();
		return coup;
	    }
	}
 ...
 */
	...
	{
	    ASSERTNOTNULL(it->second);
	    Coupling_sp coup = it->second.lock();
