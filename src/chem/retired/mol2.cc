...
AtomInfo	one;
	...
	out << ai->_SybylType << " ";
	out << a->containedBy().lock()->getTempFileId() << " ";
	out << a->containedBy().lock()->getName() << "_"
		<< a->containedBy().lock()->getTempFileId() << " ";
...
//	out << id << " ";
	...
	out << ri->firstAtom << " ";
	out << "RESIDUE 1 ";
	out << downcast<Molecule_O>(r->containedBy().lock())->getName() << " ";
	out << r->getName() << " 1";
	out << std::endl;
	id++;
    }
...
{_G();
    sybylRules = chem::TypeAssignmentRules_O::create();
