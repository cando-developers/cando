...
struct rosetta_nonbond_term {
  ...
  rosetta_nonbond_term(const rosetta_nonbond_parameters& params, double a, double c, int i3x1, int i3x2) {
    this->i3x1 = i3x1;
    this->i3x2 = i3x2;
    double sigma6 = (2.0 * a * (1.0 / (c)));
    double sigma = pow(sigma6, (1.0 * (1.0 / (6.0))));
    this->sigma = sigma;
    double epsilon = (c * c * (1.0 / ((4.0 * a))));
    this->epsilon = epsilon;
    double rrep = (0.6 * sigma);
    ...
    double rep_rrep = (epsilon * (1.0 + sr12_rep + (-2.0 * sr6_rep)));
    double drep_rrep = (epsilon * ((-12.0 * sr12_rep) + (12.0 * sr6_rep)) * (1.0 / (rrep)));
    ...
    double atr_sw = (epsilon * (sr12_sw + (-2.0 * sr6_sw)));
    double d_atr_sw = (epsilon * ((-12.0 * sr12_sw) + (12.0 * sr6_sw)) * (1.0 / (params.rswitch)));
    double dr = (params.rcut + (-(params.rswitch)));
    double aa_shift = (((2.0 * atr_sw) + (dr * d_atr_sw)) * (1.0 / ((dr * dr * dr))));
    double bb_shift = (((-2.0 * dr * d_atr_sw) + (-3.0 * atr_sw)) * (1.0 / ((dr * dr))));
    double cc_shift = d_atr_sw;
    double dd_shift = atr_sw;
    double aa = aa_shift;
    this->aa = aa;
    double bb = (bb_shift + (-3.0 * aa_shift * params.rswitch));
    this->bb = bb;
    double cc = (cc_shift + (-2.0 * bb_shift * params.rswitch) + (3.0 * aa_shift * params.rswitch * params.rswitch));
    this->cc = cc;
