...
namespace chem {
 ...
 /*!Use a GC compatible allocator here */
  template <class T>
    ...
    return gc::RootClassAllocator<Hold<T>>::untagged_allocate(obj);// return new Hold<T>(obj);
  }

  template <class T>
    Hold<T>* createHolder() {
    return gc::RootClassAllocator<Hold<T>>::untagged_allocate();
//   return new Hold<T>();
  }
 ...
 /*! Use a corresponding GC compatible deallocator here */
  template <class T>
    ...
    gc::RootClassAllocator<Hold<T>>::untagged_deallocate(objP);
//  delete objP;
  };
